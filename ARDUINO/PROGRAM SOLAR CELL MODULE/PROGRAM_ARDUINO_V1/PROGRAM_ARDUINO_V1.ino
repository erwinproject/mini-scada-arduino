...
void ProgRTC() {
  ...
  if (StatusAlat == "On") {
    ...
    if ((persen_aki < autocharge) && ((GetHour >= jamawalcharge) && (GetHour <= jamakhircharge)) && (battraifull == false)) {
      digitalWrite(MyRelay, RHigh);
      ChargeAki = "Charging";

    } else if ((persen_aki > autocharge) && ((GetHour <= jamawalcharge) && (GetHour >= jamakhircharge)) && (battraifull == true)) {
      digitalWrite(MyRelay, RLow);
